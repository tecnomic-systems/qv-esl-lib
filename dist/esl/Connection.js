...
class Connection extends eventemitter2_1.EventEmitter2 {
    ...
    bridgeToUser(endpoint) {
        return new Promise(res => {
            this.execute('bridge', `user/${endpoint}`, _ => res());
        });
    }
    bridgeToUsers(endpoints, grouping = ',') {
        const userList = endpoints.map(x => `user/${x.trim()}`).join(grouping);
        return new Promise(res => {
            this.execute('bridge', userList, _ => res());
        });
    }
    bridgeToGroup(groupName) {
        return new Promise(res => {
            this.execute('bridge', `group/${groupName}`, _ => res());
        });
    }
    ...
    record(path, timeLimitSec, silenceThreshold = 200, silenceHits = 3) {
        return new Promise(res => {
            this.execute('record', `${path} ${timeLimitSec} ${silenceThreshold} ${silenceHits}`, _ => res(true));
        });
    }
    reloadXml() {
        return new Promise(res => {
